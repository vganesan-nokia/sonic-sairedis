    ...
    m_dbPersistence(dbPersistence)
{
    ...
    if (m_dbPersistence)
    {
        /* Use default connection (Unix socket) to match orchagent's ASIC_DB usage. */
        ...
        SWSS_LOG_NOTICE("ZeroMQChannel dbPersistence enabled for %s", dbName.c_str());
    }
